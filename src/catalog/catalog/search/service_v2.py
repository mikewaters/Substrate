"""catalog.search.service_v2 - V2 Search service with full QMD feature parity.

Provides the unified search orchestrator for RAG v2, integrating:
...
Example usage:
    ...
    results = search_v2(SearchCriteria(
...
__all__ = ["SearchServiceV2", "search_v2"]
...
class SearchServiceV2:
    ...
    def __init__(self, session: "Session") -> None:
        ...
        self._hybrid_retriever_factory: "HybridRetrieverV2 | None" = None
    ...
    def _ensure_hybrid_retriever(self) -> "HybridRetrieverV2":
        """Lazy-load HybridRetrieverV2 factory."""
        if self._hybrid_retriever_factory is None:
            from catalog.search.hybrid_v2 import HybridRetrieverV2

            logger.debug("Lazy-loading HybridRetrieverV2")
            self._hybrid_retriever_factory = HybridRetrieverV2(self.session)
    ...
    def search(self, criteria: SearchCriteria) -> SearchResults:
        """Execute v2 search with query expansion, weighted RRF, and caching.
        ...
        logger.debug(
            f"SearchServiceV2.search({criteria.mode}) for '{criteria.query[:50]}...' "
    ...
    def search_hybrid(
        ...
        # Build retriever
        retriever = hybrid_factory.build(dataset_name=dataset_name, top_n=limit)
...
def search_v2(criteria: SearchCriteria) -> SearchResults:
    ...
    Convenience function that creates a session and SearchServiceV2
