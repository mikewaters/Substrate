...
from index.search.hybrid import WeightedRRFRetriever
...
class TestHybridSearchIntegration:
    ...
    def test_hybrid_search_combines_sources(
        ...
        # Sync documents (ingest + index) so FTS is populated
        config = SourceDirectoryConfig(
            source_path=sample_docs,
            dataset_name="test-vault",
            patterns=["**/*.md"],
        )
        result = DatasetSync().sync(config)
        assert result.ingest.documents_created == 3
    ...
    def test_hybrid_search_respects_dataset_filter(
        ...
        # Sync both (ingest + index) so FTS is populated
        sync = DatasetSync()
        sync.sync(
...
class TestEndToEndFlow:
    ...
    async def test_full_pipeline_ingest_search_rerank(
        ...
        # 1. Sync documents (ingest + index) so FTS is populated
        config = SourceDirectoryConfig(
            source_path=sample_docs,
            dataset_name="test-vault",
            patterns=["**/*.md"],
...
class TestResultShapeStability:
    ...
    def test_scores_dict_preserved_through_operations(self) -> None:
        ...
        from agentlayer.llm.reranker import RerankScore, blend_scores
