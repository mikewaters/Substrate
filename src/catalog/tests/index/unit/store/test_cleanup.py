...
from index.store.fts_chunk import FTSChunkManager, create_chunks_fts_table
...
class TestIndexCleanup:
    ...
    def test_reconcile_inactive_documents_removes_document_fts(
        self, cleanup: IndexCleanup, fts: FTSManager, db_session, sample_dataset: Dataset
    ) -> None:
        """reconcile_inactive_documents deletes document FTS rows for inactive docs only."""
        inactive_doc = Document(
            parent_id=sample_dataset.id,
            uri="document:test-dataset/inactive.md",
            path="inactive.md",
        ...
        stats = cleanup.reconcile_inactive_documents(
            parent_id=sample_dataset.id,
        ...
        assert isinstance(stats, ReconciliationStats)
        assert stats.documents_reconciled == 1
        assert stats.document_fts_deleted == 1
        assert stats.chunk_fts_deleted == 0
        assert stats.vector_docs_deleted == 0
        assert fts.count() == 0

    def test_reconcile_inactive_documents_removes_chunk_fts(
        self, cleanup: IndexCleanup, db_session, sample_dataset: Dataset
    ) -> None:
        """reconcile_inactive_documents deletes chunk FTS rows via source_doc_id."""
        from index.store.fts_chunk import FTSChunkManager

        inactive_doc = Document(
            parent_id=sample_dataset.id,
            uri="document:test-dataset/removed.md",
            path="removed.md",
            content_hash="h1",
            body="Content",
            active=False,
        ...
        fts_chunk = FTSChunkManager(db_session)
        source_doc_id = f"{sample_dataset.name}:removed.md"
        fts_chunk.upsert("node1", "Chunk text", source_doc_id)
        db_session.flush()
        ...
        assert stats.documents_reconciled == 1
        assert stats.chunk_fts_deleted == 1

    def test_reconcile_inactive_documents_idempotent(
        self, cleanup: IndexCleanup, fts: FTSManager, db_session, sample_dataset: Dataset
    ) -> None:
        """Repeated reconciliation is idempotent; second run removes nothing."""
        inactive_doc = Document(
            parent_id=sample_dataset.id,
            uri="document:test-dataset/inactive.md",
            path="inactive.md",
        ...
        stats1 = cleanup.reconcile_inactive_documents(
            parent_id=sample_dataset.id,
            dataset_name=sample_dataset.name,
            vector_manager=None,
        )
        db_session.flush()
        assert stats1.document_fts_deleted == 1

        stats2 = cleanup.reconcile_inactive_documents(
            parent_id=sample_dataset.id,
            dataset_name=sample_dataset.name,
            vector_manager=None,
        )
        assert stats2.documents_reconciled == 1
        assert stats2.document_fts_deleted == 0

    def test_reconcile_inactive_documents_ignores_active(
        self, cleanup: IndexCleanup, fts: FTSManager, db_session, sample_dataset: Dataset
    ) -> None:
        """Active documents are not touched by reconciliation."""
        active_doc = Document(
            parent_id=sample_dataset.id,
            uri="document:test-dataset/active.md",
            path="active.md",
        ...
        assert stats.documents_reconciled == 0
        assert stats.document_fts_deleted == 0
        assert fts.count() == 1
