"""Tests for agentlayer.llm_cache module."""
...
from agentlayer.database import create_engine_for_path
from agentlayer.llm_cache import LLMCache, LLMCacheBase, LLMCacheEntry
...
class TestLLMCacheEntry:
    ...
    def test_model_creates_table(self, tmp_path: Path) -> None:
        ...
        LLMCacheBase.metadata.create_all(engine)
...
class TestLLMCacheTTL:
    ...
    def test_ttl_defaults_to_class_constant(self, db_session) -> None:
        """TTL defaults to DEFAULT_TTL_HOURS when not provided."""
        cache = LLMCache(db_session)
        assert cache._ttl_hours == LLMCache.DEFAULT_TTL_HOURS
