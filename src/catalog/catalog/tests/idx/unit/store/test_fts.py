"""Tests for catalog.store.fts module."""
...
class TestFTSManager:
    ...
    def test_search_basic(
        ...
        doc1 = Document(
            ...
            uri="document:test-dataset/hello.md",
            path="hello.md",
            content_hash="h1",
        ...
        doc2 = Document(
            parent_id=sample_dataset.id,
            uri="document:test-dataset/goodbye.md",
            path="goodbye.md",
            content_hash="h2",
            body="Goodbye world",
        )
    ...
    def test_search_with_parent_filter(
        self, fts_manager: FTSManager, db_session
    ) -> None:
        """Search can be filtered by parent."""
        # Create two datasets
        ds1 = Dataset(
        ...
        doc1 = Document(
            parent_id=ds1.id,
            uri="document:ds1/doc1.md",
            path="doc1.md",
            content_hash="h1",
            body="Apple pie recipe",
        )
        doc2 = Document(
            parent_id=ds2.id,
            uri="document:ds2/doc2.md",
            path="doc2.md",
            content_hash="h2",
            body="Apple cider recipe",
        )
        db_session.add_all([doc1, doc2])
        db_session.flush()
        ...
        # Search in ds1 only
        results = fts_manager.search("apple", parent_filter=ds1.id)
