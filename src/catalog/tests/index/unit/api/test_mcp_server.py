...
from index.api.mcp.server import MCPServer
...
class TestMCPServerEnsureInitialized:
    ...
    def test_ensure_initialized_creates_service(self) -> None:
        ...
        with patch("index.api.mcp.server.get_session") as mock_get_session:
            mock_session = MagicMock()
            mock_get_session.return_value.__enter__ = MagicMock(return_value=mock_session)

            with patch("index.api.mcp.server.use_session"):
                with patch("index.api.mcp.server.SearchService") as mock_service_cls:
                    mock_service = MagicMock()
                    mock_service_cls.return_value = mock_service

                    with patch("index.api.mcp.server.create_mcp_tools") as mock_create_tools:
...
class TestMCPServerHandleRequest:
    ...
    def test_handle_resources_list(self, initialized_server: MCPServer) -> None:
        ...
        with patch("index.api.mcp.server.list_resources") as mock_list:
    ...
    def test_handle_resources_read(self, initialized_server: MCPServer) -> None:
        ...
        with patch("index.api.mcp.server.read_resource") as mock_read:
...
class TestRunMcpServer:
    ...
    def test_run_mcp_server_creates_and_starts(self) -> None:
        ...
        from index.api.mcp.server import run_mcp_server

        with patch("index.api.mcp.server.MCPServer") as mock_cls:
