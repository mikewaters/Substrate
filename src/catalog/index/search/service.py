"""index.search.service - Search service with full QMD feature parity.
...
from index.search.models import SearchCriteria, SearchResult, SearchResults
from agentlayer.llm_cache import LLMCache

if TYPE_CHECKING:
    from sqlalchemy.orm import Session

    from agentlayer.llm.reranker import CachedReranker
    from index.search.fts_chunk import FTSChunkRetriever
    from index.search.hybrid import HybridRetriever
    from index.search.postprocessors import (
        PerDocDedupePostprocessor,
        TopRankBonusPostprocessor,
    )
    from index.search.query_expansion import QueryExpansionTransform
    from index.search.vector import VectorSearch
...
class SearchService:
    ...
    def search(self, criteria: SearchCriteria) -> SearchResults:
        ...
        if criteria.mode in ("hybrid", "fts"):
            from index.search.intent import classify_intent
    ...
    def _build_snippet(self, chunk_text: str | None, doc_path: str) -> "SnippetResult | None":
        ...
        from index.search.formatting import build_snippet
