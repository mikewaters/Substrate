...
class TestCatalogGetTool:
    ...
    def test_catalog_get_with_dataset_prefix(self, mock_service: SearchService) -> None:
        ...
        with patch("index.api.mcp.tools.DatasetService") as mock_ds_cls:
            mock_ds = MagicMock()
...
class TestCatalogStatusTool:
    ...
    def test_catalog_status_returns_health_info(self, mock_service: SearchService) -> None:
        ...
        with patch("index.api.mcp.tools.check_health", return_value=mock_health):
            with patch("index.api.mcp.tools.DatasetService") as mock_ds_cls:
                mock_ds = MagicMock()
    ...
    def test_catalog_status_handles_error(self, mock_service: SearchService) -> None:
        ...
        with patch("index.api.mcp.tools.check_health", side_effect=Exception("Check failed")):
            tools = create_mcp_tools(mock_service)
            tool_map = {t.metadata.name: t for t in tools}
            status_tool = tool_map["catalog_status"]
