"""Tests for catalog.search.hybrid_v2 module.

Tests for WeightedRRFRetriever and HybridRetrieverV2 classes.
"""
...
class TestHybridRetrieverV2:
    """Tests for HybridRetrieverV2 factory class."""
    ...
    def test_init_default_vector_manager(self, mock_session: MagicMock) -> None:
        """Test HybridRetrieverV2 creates default VectorStoreManager."""
        with patch("catalog.search.hybrid_v2.VectorStoreManager") as mock_vm_cls, \
             patch("catalog.search.hybrid_v2.get_settings") as mock_get_settings:
            mock_vm_cls.return_value = MagicMock()
            mock_get_settings.return_value.rag_v2 = MagicMock()

            HybridRetrieverV2(mock_session)
    ...
    def test_init_custom_vector_manager(
        ...
        """Test HybridRetrieverV2 accepts custom VectorStoreManager."""
        with patch("catalog.search.hybrid_v2.get_settings") as mock_get_settings:
            mock_get_settings.return_value.rag_v2 = MagicMock()

            factory = HybridRetrieverV2(mock_session, mock_vector_manager)
    ...
    def test_build_returns_weighted_rrf_retriever(
        ...
        with patch("catalog.search.hybrid_v2.get_settings") as mock_get_settings, \
             patch("catalog.search.hybrid_v2.FTSChunkRetriever"):
            mock_get_settings.return_value.rag_v2 = mock_settings
    ...
    def test_build_uses_settings_values(
        ...
        """Test build() uses values from rag_v2 settings."""
        with patch("catalog.search.hybrid_v2.get_settings") as mock_get_settings, \
             patch("catalog.search.hybrid_v2.FTSChunkRetriever") as mock_fts_cls:
            mock_get_settings.return_value.rag_v2 = mock_settings
