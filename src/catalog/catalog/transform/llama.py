...
from catalog.store.models import Document, DocumentLinkKind
...
class LinkResolutionTransform(TransformComponent):
    ...
    ``doc_id`` in its node metadata. Reads ``_obsidian_wikilinks`` from each
    node, looks up the target document by filename stem, and creates
    ...
    _dataset_id: int = 0
    _wikilinks_key: str = "_obsidian_wikilinks"
    ...
    def __init__(
        ...
        dataset_id: int,
        *,
        wikilinks_key: str = "_obsidian_wikilinks",
        **kwargs: Any,
    ) -> None:
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        for node in nodes:
            if not node.metadata:
                continue
            wikilinks = node.metadata.get(self._wikilinks_key)
            if not wikilinks or not isinstance(wikilinks, list):
                continue
            self._process_node(node, stem_to_id, link_repo, session)
    ...
    def _process_node(
        ...
        wikilinks: list[str] = node.metadata[self._wikilinks_key]
