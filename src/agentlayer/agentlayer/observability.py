"""agentlayer.observability - Langfuse observability integration.
...
def configure_observability() -> bool:
    ...
    from agentlayer.settings import get_settings

    settings = get_settings()

    # Check if Langfuse is enabled (langfuse settings are optional)
    if not hasattr(settings, "langfuse") or not settings.langfuse.enabled:
