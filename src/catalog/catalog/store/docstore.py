"""catalog.store.llama - LlamaIndex document store backed by SQLite.
...
Example usage:
    from catalog.store.llama import SQLDocStore
    from catalog.store import get_session_factory
...
from catalog.store.models import Document as SQLDocument
...
class SQLiteDocumentStore(BaseDocumentStore):
    ...
    def parent_id(self) -> int:
        """Get the parent resource ID this store is scoped to."""
    ...
    def docs(self) -> Dict[str, BaseNode]:
        ...
        try:
            ...
            sql_docs = repo.list_by_parent(self._parent_id, active_only=True)
    ...
    def add_documents(
        ...
        try:
            ...
            for node in docs:
                existing = repo.get_by_path(self._parent_id, node.node_id)
    ...
    def get_document(
        ...
        try:
            repo = DocumentRepository(session)
            sql_doc = repo.get_by_path(self._parent_id, doc_id)
